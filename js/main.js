...
var halfmoon = {
    ...
    setBackgroundImageCheckBox: false,
...
function halfmoonOnDOMContentLoaded() {
    ...
    // * Set Time
    if (halfmoon.readLocalStorage('showTime') != null) {
        if (halfmoon.readLocalStorage('showTime') == 'true') {
            $("#time").removeClass('hidden');
            document.getElementById("showClock").checked = true;
        }
        else if (halfmoon.readLocalStorage('showTime') == 'false') {
        ...
        $("#time").removeClass('hidden');
        halfmoon.createLocalStorage('showTime', 'true');
        document.getElementById("showClock").checked = true;
    }

    // * Set Greeting
    if (halfmoon.readLocalStorage('showGreeting') != null) {
        if (halfmoon.readLocalStorage('showGreeting') == 'true') {
            $("#greetingInputDiv").removeClass('hidden');
            document.getElementById("showGreeting").checked = true;
            document.getElementById("userName").value = halfmoon.readLocalStorage('UserName');
            $("#greetingDiv").removeClass('hidden');
        }
        else if (halfmoon.readLocalStorage('showGreeting') == 'false') {
            $("#greetingInputDiv").addClass('hidden');
            $("#greetingDiv").addClass('hidden');
            document.getElementById("showGreeting").checked = false;
        }
    }
    else {
        $("#greetingDiv").addClass('hidden');
        $("#greetingInputDiv").addClass('hidden');
        halfmoon.createLocalStorage('showGreeting', 'false');
        document.getElementById("showGreeting").checked = false;
    }

    // * Set QuickLinks
    if (halfmoon.readLocalStorage('showQuickLinks') != null) {
        if (halfmoon.readLocalStorage('showQuickLinks') == 'true') {
            $("#qlinks").removeClass('hidden');
            $("#addNewLink_form").removeClass('hidden');
            document.getElementById("QuickLinks").checked = true;
            // TODO Show Quick Links List here
            // document.getElementById("userName").value = halfmoon.readLocalStorage('UserName');
        }
        else if (halfmoon.readLocalStorage('showQuickLinks') == 'false') {
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    $('.file-names').addClass('text-break');
    $('#toggleSidebar').click(function () {
        halfmoon.toggleSidebar();
        // if ($('.page-wrapper').attr("data-sidebar-hidden")) {
        //     $(this).html(
        //         '<svg class="tw-w-6 tw-h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>'
        //     );
        // } else {
        //     $(this).html(
        //         '<svg class="tw-w-6 tw-h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg>'
        //     );
        // }
    });

    // let btn_dark = document.getElementById('Dark');
    // let btn_light = document.getElementById('Light');
    ...
    $('#themeForm input').on('change', function () {
        console.log($('input[name=Theme]:checked', '#themeForm').val());
        switch ($('input[name=Theme]:checked', '#themeForm').val()) {
            case 'Dark':
                halfmoon.enableDarkMode();
                $('#Dark').checked = true;
                break;
            case 'Light':
                halfmoon.enableLightMode();
                $('#Light').checked = true;
                break;
    ...
    $('#BackgroundForm #showBgImage').on('change', function () {
        if ($("#showBgImage").is(':checked')) {
            $("#bgImageDiv").removeClass('hidden');
            halfmoon.createLocalStorage('setBackgroundImageCheckBox', true);

            // Set body background image if exist
    ...
    $('#ClockForm #showClock').on('change', function () {
        if ($("#showClock").is(':checked')) {
            $("#time").removeClass('hidden');
            halfmoon.createLocalStorage('showTime', 'true');
        }
        else {
            $("#time").addClass('hidden');
            halfmoon.createLocalStorage('showTime', 'false');
        }
    });

    $('#GreetingForm #showGreeting').on('change', function () {
        if ($("#showGreeting").is(':checked')) {
            $("#greetingInputDiv").removeClass('hidden');
            $("#greetingDiv").removeClass('hidden');
            halfmoon.createLocalStorage('showGreeting', 'true');
        }
        else {
            $("#greetingInputDiv").addClass('hidden');
            $("#greetingDiv").addClass('hidden');
    ...
    $('#QuickLinksForm #QuickLinks').on('change', function () {
        if ($("#QuickLinks").is(':checked')) {
            $("#addNewLink_form").removeClass('hidden');
            $("#qlinks").removeClass('hidden');
            halfmoon.createLocalStorage('showQuickLinks', 'true');
        }
        else {
            $("#addNewLink_form").addClass('hidden');
            $("#qlinks").addClass('hidden');
    ...
    // let btn_mode = document.getElementById('toggle-mode');
    // let btn_sidebar = document.getElementById('toggle-sidebar');

    // btn_mode.onclick = function () { halfmoon.toggleDarkMode(); };
    // btn_sidebar.onclick = function () { halfmoon.toggleSidebar(); };


});

$('.sidebar-overlay').click(function () { halfmoon.toggleSidebar(); });
...
// Configure Greeting
$('#usernameSaveBtn').click(function () {
    var username = $('#userName').val();
    halfmoon.createLocalStorage('UserName', username);
    GreetingUser();
});
